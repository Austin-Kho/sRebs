...
class Project(models.Model):
    ...
    is_direct_manage = models.BooleanField('직영운영여부', default=False, help_text='본사 직접 운영하는 프로젝트인 경우 체크')
...
class ContractUnit(models.Model):
    ...
    contract = models.OneToOneField('rebs_contract.Contract', on_delete=models.SET_NULL, null=True, blank=True, verbose_name='계약')
...
class UnitNumber(models.Model):
    ...
    unit_type = models.ForeignKey(UnitType, on_delete=models.CASCADE, verbose_name='타입')
    floor_type = models.ForeignKey('UnitFloorType', on_delete=models.SET_NULL, null=True, blank=True, verbose_name='층범위 타입')
    bldg_no = models.CharField('동', max_length=5, blank=True)
    bldg_unit_no = models.CharField('호수', max_length=5, blank=True)
    contract_unit = models.OneToOneField(ContractUnit, on_delete=models.SET_NULL, null=True, verbose_name='계약유닛')
...
class SiteOwner(models.Model):
    ...
    phone1 = models.CharField('주연락처', max_length=13)
...
class SiteContract(models.Model):
    ...
    inter_pay1 = models.PositiveIntegerField('중도금1', null=True, blank=True)
    ...
    inter_pay2 = models.PositiveIntegerField('중도금2', null=True, blank=True)
    ...
    remain_pay = models.PositiveIntegerField('잔금')
    ...
    def __str__(self):
        return f'{self.owner.owner} - [{self.total_prices}]'
